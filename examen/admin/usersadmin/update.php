...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($username_err) && empty($password_err) && empty($email_err)  && empty($telefoon_err) && empty($datum_err) && empty($geslacht_err)){
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            // probeer prepared statement uit te voeren
            if(mysqli_stmt_execute($stmt)){
                // records succesfol geupdate. redirect naar landings pagina
    ...
    // kijk of id parameter bestaat voordat je doorgaat
    if(isset($_GET["id"]) && !empty(trim($_GET["id"]))){
        // Get URL parameter
        $id =  trim($_GET["id"]);
        
        // Prepareer een select statement
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_num_rows($result) == 1){
                    ...
                    // URL heeft geen geldige ID redirect
        ...
        // URL heeft geen id statement. Redirect naar error page
        header("location: error.php");
        exit();
    }
}
